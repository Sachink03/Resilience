...
$request->validate([
    'amount' => [
        ...
        function ($attr, $value, $fail) {
            if ($value % 100 !== 0) {
                $fail('Amount must be multiple of 100');
            }
        },
    ],
]);
    ...
    $userId   = auth()->user();
    $refId = $userId->username;

    $url = 'https://api.cryptapi.io/bep20/usdt/create/';
    $queryParams = [
        'callback'      => 'https://uniqueworld.live/dynamicUpiCallback?refid=' . $refId,
        'address'       => '0x027F453fe1025DfD7aB87Dd961118CA44bbc6Bd7',
         'value'    => $amount,
        'pending'       => 0,
        'confirmations' => 1,
        'email'         => 'string',
...
$this->data['data'] = [
        ...
        'amount'        => $amount,
        'qrData'        => $data['address_in'].'?amount='.$amount,
    ];
...

{
    ...
    public function confirmDepositr(Request $request)
    {
     $userInfo = auth()->user(); // Get authenticated user
  ...
  try{
    ...
    if($validation->fails()) {
       ...
       $user_detail=User::where('username',$user->username)->orderBy('id','desc')->limit(1)->first();
        ...
        $isPayfee = Payment::where('user_id',$user_detail->id)->where('status','approved')->first();
        if (!$isPayfee) 
          {
          return redirect()->route('user.invest')->withErrors(array('you need to pay the registration fee before making an investment'));
         }

       $checkPending=Investment::where('user_id',$user_detail->id)->where('status','Pending')->orderBy('id','desc')->first();
       if($checkPending)
       {
         return redirect()->route('user.invest')->withErrors(array('you have already pending investment please wait until it is approved'));
       }
      

       $isWithdrawn=Investment::where('user_id',$user_detail->id)->where('status','Active')->where('isWithdrawn',0)->orderBy('id','desc')->first();
        if($isWithdrawn)
        {
          return redirect()->route('user.invest')->withErrors(array('you have already active investment please withdraw it first to make new investment'));
        }
