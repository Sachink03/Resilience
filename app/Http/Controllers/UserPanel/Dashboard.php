...
if($user->active_status!="Pending")
{
 ...
 $pckage = $user->package * 5 ;   
 $sdate = $user->booster_adate;
 $newDate =date('Y-m-d H:i:s', strtotime($sdate. ' + 10 days'));

        $directsIn7DaysSamePkg = User::where('sponsor', $user->id)
            ->where('active_status', 'Active')->whereBetween('adate', [$sdate,$newDate])
            ->sum('package');   
            
  $this->data['isBooter'] =($directsIn7DaysSamePkg >= $pckage) ? true : false ;          
}
     ...
     public function submitRegistrationPayment(Request $request)
    {
        $request->validate([
        ...
        if (Payment::where('txid', $request->txid)->exists()) {
            return response()->json([
                'success' => false,
                'message' => 'This transaction ID has already been used. Please check and try again.'
            ]);
        }

        // Prevent duplicate submission
        if (Payment::where('user_id', $user->id)->where('status', 'pending')->exists()) {
            return response()->json([
                'success' => false,
                'message' => 'You already have a pending payment. Please wait for verification.'
            ]);
        }
        ...
        // Optional: mark user as "Under Review" for activation
        $user->update(['status' => 'Under Review']);

        return response()->json([
            'success' => true,
            'message' => 'Payment submitted successfully. Your account will be activated after verification.',
        ]);
    }
