    ...
    public function index(Request $request)
    {
       $user=Auth::user();
          ...
          $limit = $request->limit ? $request->limit : paginationLimit();
            $status = $request->status ? $request->status : null;
            $search = $request->search ? $request->search : null;
            $notes = Income::where('user_id',$user->id)->where('remarks','Passive Income')->orderBy('id', 'DESC');
           if($search <> null && $request->reset!="Reset"){
            $notes = $notes->where(function($q) use($search){
              $q->Where('rname', 'LIKE', '%' . $search . '%')
              ->orWhere('ttime', 'LIKE', '%' . $search . '%')
...
public function reward_income(Request $request)
{
    ...
    // User ke sab level fetch karo as array
    $user_levels = Reward::where('user_id', $user->id)->pluck('level')->toArray();

    // Level 1 se 7 tak status set karo
    for ($i = 1; $i <= 15; $i++) {
        $this->data["level{$i}_status"] = in_array($i, $user_levels) ? 'Achieved' : 'Pending';
    }
    ...
    public function roi_income(Request $request)
    {
          ...
          $limit = $request->limit ? $request->limit :  paginationLimit();
            $status = $request->status ? $request->status : null;
            $search = $request->search ? $request->search : null;
            $notes = Income::where('user_id',$user->id)->where('remarks','Profit Sharing Income')->orderBy('id', 'DESC');
           if($search <> null && $request->reset!="Reset"){
            $notes = $notes->where(function($q) use($search){
              $q->Where('ttime', 'LIKE', '%' . $search . '%')
              ->orWhere('amt', 'LIKE', '%' . $search . '%')
