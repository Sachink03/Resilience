   ...
   public function change_price(Request $request)
    {
        ...
        try {
            $data = $request->all();
            $rules = array('percentage' => 'required','walletAddress' => 'required');
            $msg = [
                'percentage.required'     => 'percentage is required',
                'walletAddress.required'     => 'percentage is required',
            ...
            if ($validator->fails())
              ...
              \DB::table('general_settings')->where('id',1)->update(['roiPercentage'=>$request->percentage,'usdtBep20'=>$request->walletAddress]);
