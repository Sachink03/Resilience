...
function sendAdminEmail($type = null, $shortCodes = [])
{
    $adminEmail = 'Raj@gmail.com';

    $mail_data = [
        'subject'    => $type,
        'MailDetail' => $shortCodes
    ];

    \Mail::to($adminEmail)->send(new Sendmail($mail_data));
}
...
$user_mid = $data->id;
      ...
      $amount = $amt/100;
        ...
        while ($user_mid!="" && $user_mid!="1"){
              ...
              $pp=0;
               if($sp_status=="Active" && $Sposnor_cnt>=$cnt)
               {
                 if($cnt==1)
                  {
                    $pp= $amount*10;
